...
if name_input:  # 이름이 입력되었을 때 바로 처리
    if len(st.session_state.participants) >= 10:
        ...
        # 결과 확인 자동 실행
        if len(st.session_state.participants) == 10 and st.session_state.current_index < len(st.session_state.participants):
            participant = st.session_state.participants[st.session_state.current_index]
            
            # 등수 조건에 따라 경품 할당
            if st.session_state.current_index < 3:  # 1~3번째 참가자는 3등만 가능
                if not st.session_state.winners["3%"]:
                    prize = "3%"
                else:
                    prize = "2%"
            elif 3 <= st.session_state.current_index < 6:  # 4~6번째 참가자는 2등만 가능
            ...
            with col1:
                if prize == "100%":
                    ...
                    st.balloons()
                    st.session_state.winners["100%"] = participant
                    ...
                    st.snow()
                    st.session_state.winners["20%"] = participant
                    ...
                    st.session_state.winners["3%"] = participant
                    ...
                    st.session_state.winners["2%"].append(participant)
